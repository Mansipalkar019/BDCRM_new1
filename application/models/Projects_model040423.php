 ...
 return $fData;
   ...
   //  }
    ...
    function getStaffInfoDetails($project_id,$company_name){
        $designation_name = $this->session->userdata('designation_name');
          $user_id = $this->session->userdata('id');
        $this->db->select('bmp.*,buf.first_name,buf.last_name,buf.has_replaced,buf.caller_has_replaced,buf.updated_status,buf.received_company_name as comp_name,buf.project_id,buf.id,bswd.dispositions,bsvd.id as vdid,bsvd.voice_dispositions,bmps.project_type,buf.validation_status,bc.name as country_name,buf.activity_type');
        $this->db->from('bdcrm_uploaded_feildss buf');
        $this->db->join('bdcrm_company_disposition bmp','buf.company_disposition = bmp.id','left');  
        $this->db->join('bdcrm_staff_web_disposition bswd','buf.web_staff_disposition=bswd.id','left');
        $this->db->join('bdcrm_staff_voice_dispositions bsvd','buf.voice_staff_disposition=bsvd.id','left');  
    ...
    function getAllStaffInfoDetails($project_id){
         $this->db->select('bmp.*,buf.first_name,buf.last_name,buf.updated_status,buf.received_company_name as comp_name,buf.project_id,buf.id,bswd.dispositions,bsvd.voice_dispositions,bmps.project_type,buf.validation_status,bc.name as country_name');
        $this->db->from('bdcrm_uploaded_feildss buf');
        $this->db->join('bdcrm_company_disposition bmp','buf.company_disposition = bmp.id','left');
        $this->db->join('bdcrm_staff_web_disposition bswd','buf.web_staff_disposition=bswd.id','left');
        $this->db->join('bdcrm_staff_voice_dispositions bsvd','buf.voice_staff_disposition=bsvd.id','left');
   ...
   function getPreLastInfo($project_id="",$rowid="",$cmp_name=""){
    ...
    function excel_download($project_id="")
    {
        ...
        buf.staff_note,buf.research_remark,buf.voice_remark,buf.researcher_company_remark,buf.caller_company_remark,buf.researcher_company_note,buf.caller_company_note,buf.has_replaced,buf.has_replaced1,
        buf.caller_has_replaced,buf.caller_has_replaced1,buf.created_date,buf.updated_at,CONCAT(us.first_name,' ',us.last_name) as updated_by,GROUP_CONCAT(ca.reassigned_to) as reassigned_to,bmp.project_name
        ...
        WHERE buf.project_id = $project_id GROUP by buf.id";
...
function getstaffwebdisbytasktype($task_type,$disposition_status,$main_staff_disposition,$first_name,$last_name,$web_staff_disposition)
 ...
 { 
   ...
   function getvoicerecord($from_date="",$to_date="",$rowno="",$rowperpage="",$search_text="",$first_name="",$last_name="")
   {
       $this->db->select('ch.*,bsvd.voice_dispositions,us.username');
   ...
   function getvoicerecord_count_filtered($from_date="",$to_date="",$rowno="",$rowperpage="",$search_text="",$first_name="",$last_name="")
   {
       $this->db->select('ch.*,bsvd.voice_dispositions,us.username');
       $this->db->from('caller_history ch');
       $this->db->join('bdcrm_staff_voice_dispositions bsvd','bsvd.id=ch.voice_staff_disposition','left');
   ...
   function getvoicerecord_count_all($from_date="",$to_date="",$rowno="",$rowperpage="",$search_text="",$first_name="",$last_name="")
   {
       $this->db->select('ch.*,bsvd.voice_dispositions,us.username');
       $this->db->from('caller_history ch');
       $this->db->join('bdcrm_staff_voice_dispositions bsvd','bsvd.id=ch.voice_staff_disposition','left');
