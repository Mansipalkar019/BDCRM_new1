        ...
        <tbody>
            <?php foreach($ProjectInfo as $key => $value){
                ...
                <tr>
                       ...
                       <td><?php if(!empty($value['sa1']) && date('d-m-Y',$value['sa1']) == '01-01-1970') { echo date(('d-m-Y h:i A'),strtotime($value['sa1'])); }else{ echo "NA" ; } ?></td>
                       <td><?php if(!empty($value['sa2']) && date('d-m-Y',$value['sa1']) == '01-01-1970'){ echo date(('d-m-Y h:i A'),strtotime($value['sa2'])); }else{ echo "NA" ; } ?></td>
                       <td><?php if(!empty($value['sa3']) && date('d-m-Y',$value['sa1']) == '01-01-1970'){ echo date(('d-m-Y h:i A'),strtotime($value['sa3'])); }else{ echo "NA" ; } ?></td>
                       <td><?php if(!empty($value['sa4']) && date('d-m-Y',$value['sa1']) == '01-01-1970'){ echo date(('d-m-Y h:i A'),strtotime($value['sa4'])); }else{ echo "NA" ; } ?></td>
                       <td><?php if(!empty($value['sa5']) && date('d-m-Y',$value['sa1']) == '01-01-1970'){ echo date(('d-m-Y h:i A'),strtotime($value['sa5'])); }else{ echo "NA" ; } ?></td>
                      ...
                      <!--   <td><?php  echo date(('d-m-Y h:i A'),strtotime($value['created_date'])); ?></td>
                       <td><?php  echo date(('d-m-Y h:i A'),strtotime($value['updated_at'])); ?></td>
                       <td><?php echo date(('d-m-Y h:i A'),strtotime($value['assigned_at'])); ?></td> -->
...
<script type="text/javascript">
   ...
   $(document).ready(function () {
    ...
    var table = $('#example').DataTable({
        ...
        // serverSide: true,
        // ajax: {
        //     url: '<?= base_url() ?>Projects/get_project_staff_info_new',
        //     type: 'POST',
        // },
