...
class Master  extends CI_Controller
{
 ...
 public function add_departments($id = 0){
    ...
    public function add_industry($id=0){
        //echo $id;die();
        if($id!=0 && $id!=''){
           $data['getFormInfo'] = $this->model->getData('bdcrm_industries', array('status' => '1','id'=>$id))[0];
        }
        $data['main_content'] = "main/add_industry";
        $data['getAllCountry'] = $this->model->getData('bdcrm_industries', array('status' => '1'));
        $this->load->view("includes/template", $data);
   }


   public function submit_industry(){
    $this->form_validation->set_rules("industry_name","Industry Name","trim|required|min_length[2]|max_length[100]|xss_clean",array("required"=>"%s is required"));
   
    if($this->form_validation->run()==true){
        $industry_id = $this->security->xss_clean($this->input->post("industry_id"));
        $industry_name = $this->security->xss_clean($this->input->post("industry_name"));
        $data = array(
            "Industries" => $industry_name,
            "status"=>1
        );
        if(empty($industry_id)){
            $addCountry = $this->model->insertData('bdcrm_industries',$data);
            if($addCountry){
                $this->session->set_flashdata("success","Industry has been successfully added.");
            }
        }else{
          $updateCountry = $this->model->updateData("bdcrm_industries", $data, array('id' => $industry_id));
          if($updateCountry){
              $this->session->set_flashdata("success","Industry has been successfullly updated");
          }
        }
    ...
    else
    {
     ...
     redirect(base_url("master/add_industry"));
}

public function delete_industry($id){
    if(!empty($id)){
       $deleteCountry = $this->model->updateData("bdcrm_industries", array('status'=>0), array('id' => $id));
      if($deleteCountry){
         $this->session->set_flashdata("success","Industry has been successfullly deleted");
      }
    }
    redirect(base_url("master/add_industry"));
}
    ...
    public function add_staff_voice_dispositions($id=0){
         ...
         $data['getStaffVoiceDispo'] = $this->Projects_model->getStaffVoiceDispo();  
        
         $data['getcoStaffVoiceDispo'] = $this->model->getData('bdcrm_caller_disposition', array('status' => '1'));  
         $data['main_content'] = "main/add_staff_voice_disposition";
         $this->load->view("includes/template", $data);
    }
    
    public function getstaffvoicdisplist()
    {
        $disp_id = $this->input->post('val');
        $vsd=$this->input->post('vsd');
        $query = $this->model->getData('bdcrm_staff_voice_dispositions', array('co_dispositions' => $disp_id)); 
        $html="<option value=''>Select</option>";
        foreach ($query as $key => $bdd) {
            if($bdd['id'] == $vsd)
            { $selected='selected'; }else{ $selected=' '; }
           $html .= "<option value='".$bdd['id']."' $selected>".$bdd['voice_dispositions']."</option>";
         }
    ...
    public function submit_staff_voice_dispositions(){
        ...
        if($this->form_validation->run()==true){
            ...
            $company_voice_dispositon = $this->security->xss_clean($this->input->post("company_voice_dispositon"));
            $data = array(
                "voice_dispositions" => $staff_voice_disposition_name,
                "co_dispositions" => $company_voice_dispositon,
            );
    ...
    public function submit_company_remark(){
        ...
        if($this->form_validation->run()==true){
            ...
            $company_remark = $this->security->xss_clean($this->input->post("company_remark"));
            $data = array(
    ...
    public function add_researcher_remark($id=0){
        
        if($id!='' AND $id!=0){
            $data['getFormProjects'] = $this->model->getData('bdcrm_researcher_remark', array('status' => '1','id'=>$id))[0];  
         }
         $data['getProjectTypes'] = $this->model->getData(' bdcrm_researcher_remark', array('status' => '1'));  
         $data['main_content'] = "main/add_researcher_remark";
         $this->load->view("includes/template", $data);
    }


    public function submit_researcher_remark(){
        
        $this->form_validation->set_rules("researcher_remark","Researcher Remark","trim|required|min_length[2]|max_length[100]|xss_clean",array("required"=>"%s is required"));
        if($this->form_validation->run()==true){
            $researcher_remark = $this->security->xss_clean($this->input->post("researcher_remark"));
            $data = array(
                "researcher_remark" => $researcher_remark,
            );

            $researcher_remark_id = $this->input->post("researcher_remark_id");
            if(empty($researcher_remark_id)){
                $addResearcherRemarks = $this->model->insertData('bdcrm_researcher_remark',$data);
                if($addResearcherRemarks){
                    $this->session->set_flashdata("success", "Researcher remark has been added.");
                }
            }else{
                $updateResearcherRemark = $this->model->updateData("bdcrm_researcher_remark", $data, array('id' => $researcher_remark_id));
                if($updateResearcherRemark){
                    $this->session->set_flashdata("success","Researcher Remark Successfullly Updated.");
                }
        ...
        else
        {
         ...
         redirect(base_url("master/add_researcher_remark"));

    }


     public function DeleteResearcherRemark($id=0){
        if($id!='' && $id!=0){
         if(!empty($id)){
            $DeleteProjectType = $this->model->updateData("bdcrm_researcher_remark", array('status'=>0),
            array('id' => $id));
            if($DeleteProjectType){
               $this->session->set_flashdata("success","Researcher Remark has been successfullly deleted.");
            }
         }
        }
       redirect(base_url("master/add_researcher_remark"));
    ...
    public function add_caller_remark($id=0){

        if($id!='' AND $id!=0){
            $data['getFormProjects'] = $this->model->getData('bdcrm_caller_remark', array('status' => '1','id'=>$id))[0];  
         }
         $data['getProjectTypes'] = $this->model->getData('bdcrm_caller_remark', array('status' => '1'));  
         $data['main_content'] = "main/add_caller_remark";
         $this->load->view("includes/template", $data);
    }


    function submit_caller_remark(){


        $this->form_validation->set_rules("caller_remark","Caller Remark","trim|required|min_length[2]|max_length[100]|xss_clean",array("required"=>"%s is required"));
        if($this->form_validation->run()==true){
            $caller_remark = $this->security->xss_clean($this->input->post("caller_remark"));
            $data = array(
                "caller_remark" => $caller_remark,
            );

            $caller_remark_id = $this->input->post("caller_remark_id");
            if(empty($caller_remark_id)){
                $addCallerRemarks = $this->model->insertData('bdcrm_caller_remark',$data);
                if($addCallerRemarks){
                    $this->session->set_flashdata("success", "Researcher remark has been added.");
                }
            }else{
                $updateCallerRemark = $this->model->updateData("bdcrm_caller_remark", $data, array('id' => $caller_remark_id));
                if($updateCallerRemark){
                    $this->session->set_flashdata("success","Caller Remark Successfullly Updated.");
                }
        ...
        else
        {
         ...
         redirect(base_url("master/add_caller_remark"));
    ...
    public function DeleteCallerRemark($id=0){
        if($id!='' && $id!=0){
         if(!empty($id)){
            $DeleteCallerRemark = $this->model->updateData("bdcrm_caller_remark", array('status'=>0),
            array('id' => $id));
            if($DeleteCallerRemark){
               $this->session->set_flashdata("success","Caller Remark has been successfullly deleted.");
            }
         }
        }
       redirect(base_url("master/add_caller_remark"));
